...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function success(data, status) {
      ...
      $(data).find('item').each(function (index) {
        ...
        var title = $item.find('title').text(); //publication date

        var pubDate = $item.find('pubDate').text(); //url from media:content

        var media = $item.find('media\\:content, content').attr('url'); // }
        //create parent div element

        var temp_parent_div = document.createElement('article'); //create date

        var temp_date = document.createElement('p');
        temp_date.innerHTML = pubDate;
        temp_date.classList.add('article_title'); //create image

        var temp_img = document.createElement('img'); //grab url for the image

        temp_img.src = media;
        temp_img.style.width = "80%";
        temp_img.style.height = "80%";
        temp_img.classList.add('project_image'); //create header

        var temp_header = document.createElement('p');
        temp_header.innerHTML = title;
        temp_header.classList.add('temp_header'); //append to parent

        temp_parent_div.append(temp_date, temp_img, temp_header); //store it in project_data

        project_data.push(temp_parent_div.outerHTML);
      }); // Initially load some items after making array

      pagemaker(); // console.log("Project_Data First Element" + " "+ project_data[0]);
    },
    error: function error(request, data, status) {
      console.log("An error occured when getting the data");
    }
  });
  var navtext = document.getElementsByClassName("navtext"); //nav text css function
  ...
  document.getElementById("reveal_nav").addEventListener("click", function () {
    var colnav = document.getElementById("c_nav"); //IYT WORKAS OMG YES
  ...
  document.getElementById("login_btn").onclick = function () {
    ...
    var micro_form = document.getElementById('micro_form'); // Toggle hidden class
    ...
    if (loginForm.style.display !== "block" && micro_form.style.display != "none") {
      ...
      micro_form.style.opacity = "1.0"; // loginForm.style.display = "none";
  ...
  var content = document.getElementById('email_writer'); //using the speech recognition api built into the window library

  var SpeechRecognition = window.SpeechRecoginition || window.webkitSpeechRecognition; //create a new speech recognition object

  var recognition = new SpeechRecognition();
  recognition.continous = true;
  var voice_running = false;
  var initial_content; //executes when speech starts
  ...
  recognition.onresult = function (e) {
    ...
    var transcript = e.results[current_text][0].transcript; //save data we got

    if (content.value.length != 0) {
      console.log(initial_content, _typeof(transcript));
  ...
  microphone.addEventListener('click', function (e) {
    e.preventDefault(); //check if the voice is already running.
  ...
  project_page.addEventListener('scroll', function () {
    ...
    if (project_page.scrollTop + project_page.clientHeight + 10 >= project_page.scrollHeight && project_page.children.length < project_data.length * 2) {
      ...
      console.log(project_data.length); // if(){

      console.log("yessss"); // this pagemaker function is not working for some reason
  ...
  $('#email_form').on('submit', function (event) {
    ...
    var email_name = document.getElementById('name').value;
    var email_address = document.getElementById('address').value;
    var email_message = document.getElementById('email_writer').value; //if there are no empty fields
    ...
    if (email_name.length != 0 && email_address.length != 0 && email_message.length != 0) {
      ...
      if (ValidateEmail(email_address)) {
        ...
        emailjs.send('service_j9ugxeh', 'template_xauovko', temp_params).then(function (response) {
...
function pagemaker() {
  ...
  project_page.appendChild(temp_spacer); //add page
  ...
  temp_col_one.id = 'col1'; //append data from rss feed
  // console.log(project_data); //accessing elements returns undefined

  temp_insta = document.createElement('div');
  temp_insta.classList.add('photo_container'); // console.log(project_data[pagenumber]);

  temp_insta.innerHTML = project_data[pagenumber];
  temp_col_one.appendChild(temp_insta); //3d effect with mousemovement, to be used for pictures.
  //source: https://www.youtube.com/watch?v=XK7T3mY1V-w&t=579s

  temp_col_one.addEventListener('mousemove', function (e) {
    e.currentTarget.style.transform = 'translate3d(' + String(e.pageX) + ',0,' + String(e.pageY) + ')'; // console.log(e.clientX, e.clientY);

    var xAxis = (window.innerWidth / 2 - e.pageX) / 70;
    var yAxis = (window.innerHeight / 2 - e.pageY) / 75; // temp_insta.style.transform = 'rotateY(' + String(yAxis) + 'deg)';
  ...
  temp_col_two.id = 'col2'; //load comments from text files to comments section

  var commentary;
  var temp_comment_holder = document.createElement('div'); //set interval
    ...
    $.ajax({
      ...
      success: function success(data, status) {
        ...
        for (var i = 0; i < new_data.length - 1; i++) {
          var temp_comment = document.createElement('div');
          temp_comment.classList.add('comments');
          temp_comment.innerText = new_data[i]; // console.log(new_data[i]);
          //add to the element
        ...
        console.log('okayman'); // setTimeout(updater,4000);
      },
      error: function error(request, data, status) {
        console.log("An error occured when getting the data");
      }
  ...
  temp_comment_section.method = 'POST'; // var user_text = document.createElement('p');
  ...
  var image_field = document.createElement('input'); // user_text.innerHTML = 'Name: ';

  user_message.placeholder = 'Username:';
  user_message.type = "text"; //send name

  user_message.name = 'name'; //message box

  message_text.placeholder = 'Send a comment!';
  message_text.rows = '3';
  message_text.cols = '10'; //send comment

  message_text.name = 'comment'; //hides this field from the user.

  image_field.type = 'hidden';
  image_field.name = 'comments_image';
  image_field.value = String(pagenumber + 1); //send button

  text_send.type = 'submit';
  text_send.innerHTML = 'send';
  temp_comment_section.append(user_message, image_field, message_text, text_send); //append form after the comments

  temp_col_two.append(temp_comment_holder, temp_comment_section); //navbar
  ...
  temp_page.appendChild(temp_project); //Add to the page
